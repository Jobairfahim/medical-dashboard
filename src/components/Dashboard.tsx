    ...
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        {(
          ...
          <div key={i} className="bg-white rounded-2xl p-3 sm:p-4 flex items-center gap-2 sm:gap-3 shadow-sm border border-gray-100 hover:shadow-md hover:-translate-y-0.5">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-teal-50 rounded-xl flex items-center justify-center text-sm sm:text-lg flex-shrink-0">
              {s.icon}
            </div>
            <div>
              <p className="text-lg sm:text-2xl font-bold text-gray-800 leading-none mb-0.5">{s.value}</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        {/* Recent placements */}
        <div className="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-5">
          ...
          <div className="mt-5 pt-4 border-t border-gray-100">
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2">
              <button type="button" onClick={() => setShowModal(true)}
                className="flex items-center justify-center gap-2 bg-teal-500 hover:bg-teal-600 text-white text-xs font-semibold px-4 py-2 rounded-xl shadow-md shadow-teal-200/50">
                <Icon path="M12 4v16m8-8H4" className="w-3.5 h-3.5" /> Create Placement
              </button>
              <button type="button" onClick={() => navigate("applications")}
                className="flex items-center justify-center gap-2 bg-teal-900 hover:bg-teal-800 text-white text-xs font-semibold px-4 py-2 rounded-xl">
                <Icon path="M15 12a3 3 0 11-6 0 3 3 0 016 0z" className="w-3.5 h-3.5" /> View Total Applications
              </button>
              <button type="button" onClick={() => navigate("settings")}
                className="flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold px-4 py-2 rounded-xl">
